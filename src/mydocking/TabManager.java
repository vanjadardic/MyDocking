...
public class TabManager extends JPanel {
   ...
   public void paint(Graphics g) {
      ...
      if (dropPoint != null) {
         ...
         if (dropLocation.tabContainer != null) {
            Dimension s = dropLocation.tabContainer.getSize();
            Insets i = dropLocation.tabContainer.getInsets();
            i.left += 1;
            i.top += 1;
            i.right += 2;
            i.bottom += 2;
   ...
   private DropLocation getDropLocation(Point dropPoint) {
      ...
      if (dropLocation.tabContainer != null) {
         ...
         size.height -= dropLocation.tabContainer.getTabs().getHeight();
         boolean isTop = false;
         if (dropPoint.y < dropLocation.tabContainer.getTabs().getHeight()) {
            dropLocation.location = DropLocation.TABS;
            isTop = true;
         } else if (dropPoint.y <= dropLocation.tabContainer.getTabs().getHeight() + size.height * 0.2f) {
            dropLocation.location = DropLocation.TOP;
         } else if (dropPoint.y >= dropLocation.tabContainer.getTabs().getHeight() + size.height * 0.8f) {
